...
public class TileEntityFuelRod extends TileEntity implements ITankContainer, IRadiationModerator {
	...
	/**
	 * @return Radiant heat generated by the fuel rod.
	 */
	public double radiate() {
		...
		radiantHeat += receivePulse(radiation);
		...
		for(int i = 1; i < 5; i++) {
			...
			if(te != null && te instanceof IRadiationModerator) {
				...
				radiantHeat += ir.receivePulse(radiation);
			}
			else {
				mat = worldObj.getBlockMaterial(xCoord + (dx*i), yCoord, zCoord+(dz*i));
				if(mat != null) {
					radiantHeat += modulateRadiationByMaterial(radiation, mat);
				}
				else {
					// Durr..?
					radiantHeat += modulateRadiationByMaterial(radiation, Material.air);
				}
			}
	 ...
	 * @return
	 */
	public double leakHeat() {
		if(localHeat <= 0.0) { return 0.0; }
		
		// Modulate local heat. Dump it into the reactor.
		double heatProduced = localHeat * 0.01;
		
		// In the case of cool fuel rods, some heat will be "lost".
		localHeat -= Math.max(1.0, heatProduced);
