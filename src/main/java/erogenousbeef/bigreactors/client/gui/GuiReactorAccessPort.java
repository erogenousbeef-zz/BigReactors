	...
	protected void actionPerformed(GuiButton button) {
		if(button.id == 0 || button.id == 1) {
			...
			if(newMetadata != metadata) {
                CommonPacketHandler.INSTANCE.sendToServer(new MultiblockNetworkHandlerMessageServer(Packets.AccessPortButton, _port.xCoord, _port.yCoord, _port.zCoord, (byte)newMetadata));
			}
		}
		
		else if(button.id == 2 || button.id == 3) {
			boolean fuel = button.id == 2;
            CommonPacketHandler.INSTANCE.sendToServer(new MultiblockNetworkHandlerMessageServer(Packets.ReactorEjectButton, _port.xCoord, _port.yCoord, _port.zCoord, fuel, isShiftKeyDown(), true, _port.xCoord, _port.yCoord, _port.zCoord));
		}
	}
