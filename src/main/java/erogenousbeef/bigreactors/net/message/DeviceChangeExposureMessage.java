...
public class DeviceChangeExposureMessage extends WorldMessageServer {
	...
	public DeviceChangeExposureMessage(int x, int y, int z, int side) {
		super(x, y, z);
		this.side = side;
	}
	...
	public static class Handler extends WorldMessageServer.Handler<DeviceChangeExposureMessage> {
		...
		protected IMessage handleMessage(DeviceChangeExposureMessage message, MessageContext ctx, TileEntity te) {
			if(te instanceof TileEntityInventory) {
				((TileEntityInventory)te).onChangeInventorySide(message.side);
			}
			else {
				BRLog.warning("Received SideChangeMessage for TE at %d, %d, %d, but it was not a TE with an iterable inventory exposure!", te.xCoord, te.yCoord, te.zCoord);
			}
			return null;
		}
	}
